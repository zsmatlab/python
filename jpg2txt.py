...
# # 直接读取单通道灰度图
gray_img = cv.imread('C:/Users/seeta/Desktop/13.jpg', cv.IMREAD_GRAYSCALE)
# gray_img = cv.resize(gray_img,(int(gray_img.shape[1]),int(gray_img.shape[0])),interpolation=cv.INTER_CUBIC)
gray_img = cv.resize(gray_img,(200,300),interpolation=cv.INTER_CUBIC)
cv.imshow('iker', gray_img)
cv.waitKey(0)
print(gray_img.shape)   # 先是高度， 再是宽度
file = open('C:/Users/seeta/Desktop/13.txt',mode='w')
